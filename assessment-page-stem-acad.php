...
$sql_academic = "SELECT question_text, a, b, c, d FROM question_stem_acad WHERE qtype = 'academic' AND assigned_course IN ($placeholders)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql_academic = "SELECT question_text, a, b, c, d, assigned_course, correct_answer FROM question_stem_acad WHERE qtype = 'academic' AND assigned_course IN ($placeholders)";
    $stmt_academic = mysqli_prepare($con, $sql_academic);
    mysqli_stmt_bind_param($stmt_academic, str_repeat('s', count($courseNames)), ...$courseNames);
    mysqli_stmt_execute($stmt_academic);
    $result_academic = mysqli_stmt_get_result($stmt_academic);
    ...
    <section class="assessment-section">
        <div class="min-width__section">
            <div class="section__headings">
                ...
                // Prepare the SQL statement for academic questions
                $placeholders = implode(', ', array_fill(0, count($courseNames), '?'));
                $sql_academic = "SELECT COUNT(*) as total_academic_questions FROM question_stem_acad WHERE qtype = 'academic' AND assigned_course IN ($placeholders)";
                $stmt_academic = mysqli_prepare($con, $sql_academic);
                mysqli_stmt_bind_param($stmt_academic, str_repeat('s', count($courseNames)), ...$courseNames);
                mysqli_stmt_execute($stmt_academic);
                $result_academic = mysqli_stmt_get_result($stmt_academic);
