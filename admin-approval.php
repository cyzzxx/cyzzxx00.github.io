...
?>
    ...
    <div class="table-wrapper" tabindex="0">
        ...
        <div class="table-wrapper__search" style="margin-bottom: 1rem;">
            <input type="text" name="" id="table-search" onkeyup="approvalTable()" placeholder="Search by First Name / LRN" class="search-input">
        ...
        <table>
            ...
            <tbody id="course-data-storage" data-filter-control="true" data-show-search-clear-button="true">
                <?php
                $sql = "SELECT * FROM user ORDER BY user_time DESC";
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    ?>
                    <tr>
                        <td data-cell="User ID"><?php echo $user_id; ?></td>
                        <td data-cell="First name"><?php echo $fname; ?></td>
                        <td data-cell="Last name"><?php echo $lname; ?></td>
                        <td data-cell="Email"><?php echo $email; ?></td>
                        <td data-cell="LRN"><?php echo $lrn; ?></td>
                        <td data-cell="Strand"><?php echo $strand_track; ?></td>
                        <td data-cell="Status"><?php echo $status; ?></td>
                        <td data-cell="Ref ID"><?php echo $ref; ?></td>
                        <td data-cell="Operation" class="operation-icons-flex">
                            <!-- For Accept User -->
                            <?php if (!$row['status']): ?>
                                <button class="td__button" class="form-controls"  onclick="showUserConfirmation('<?php echo $user_id; ?>')">
                                    <i class="bx bxs-user-check"></i>
                                </button>
                                <!-- For Reject User -->
                                <button class="td__button" onclick="showRejectConfirmation('<?php echo $user_id; ?>')">
                                    <i class="bx bxs-x-circle"></i>
                                </button>

                            <?php else: ?>
                                <button class="td__button delete-btn" onclick="showDeleteConfirmation('<?php echo $user_id; ?>')">
    ...
    <script>
        ...
        function showUserConfirmation(userId) {
            Swal.fire({
                title: 'Accept User',
                text: 'Are you sure you want to accept this record?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    // Call a function to handle accepting the user
                    confirmUser(userId);
        ...
        function showRejectConfirmation(userId) {
            Swal.fire({
                title: 'Reject User',
                ...
                if (result.isConfirmed) {
                    // Call a function to handle rejecting the user
        ...
        function confirmUser(userId) {
            ...
            console.log('Confirming user ID:', userId);

            // Perform AJAX request to handle server-side acceptance without displaying confirmation
            fetch('accept_user.php', {
                method: 'POST',
                body: JSON.stringify({ userId: userId })
            })
            .then(response => response.json())
            .then(data => {
                // Check the response to verify the acceptance status
                // No need to display a SweetAlert here if not required

                // Optionally, perform additional client-side actions after acceptance
                console.log(`User ID ${userId} has been accepted.`);
                // Redirect or perform other actions as needed
            })
            .catch(error => {
                // Display error message if there was an issue with server-side acceptance
                console.error('Failed to accept the user:', error);
            });
        }
        ...
        function confirmReject(userId) {
            ...
            .then(data => {
                ...
                // Optionally, perform additional client-side actions after acceptance
                console.log(`User ID ${userId} has been rejected.`);
                // Redirect or perform other actions as needed
            })
            .catch(error => {
                // Display error message if there was an issue with server-side acceptance
                console.error('Failed to reject the user:', error);
            });
        }
    ...
    <script>
        function showDeleteConfirmation(userId) {
            // Using SweetAlert to display a confirmation modal
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    fetch('delete-user-request.php?user_id=' + userId, {
                        method: 'GET' 
                    })
                    .then(response => {
                        Swal.fire(
